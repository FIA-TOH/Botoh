export const constants = {
  // ===============================
  // Speed and Base Performance
  // ===============================
  NORMAL_SPEED: 1,
  DEFAULT_PLAYER_RADIUS: 14.25,
  DRS_SPEED_GAIN: 0.001,
  ERS_PENALTY: -0.006,
  JUMP_START_PENALTY: -0.005,
  FULL_GAS_SPEED: 0.00025,
  ZERO_GAS_PENALTY: 0.005,
  PENALTY_SPEED: 0.97,

  // ===============================
  // Pitlane e Safety Car
  // ===============================
  DEFAULT_PIT_SPEED: 0.97,
  SAFETY_CAR_SPEED: 0.985,
  SAFETY_CAR_INDY_SPEED: 0.993,

  // ===============================
  // Slipstream
  // ===============================
  // When change MAX_SLIPSTREAM, change the max speed pressing X in Indianapolis too
  MAX_SLIPSTREAM: 0.000200, // Max of speed gains with full slipstream
  RESIDUAL_SLIPSTREAM_TIME: 3, // How much time does it take to the slipstream to end
  SLIPSTREAM_RESIDUAL_VALUE: 0.000200 * 0.2, // How much powerfull is the slipstream when you are closer
  SLIPSTREAM_ACTIVATION_DISTANCE: 400, // The distance you have to be to the slipstream start to act
  SLIPSTREAM_LATERAL_TOLERANCE: 38, // The width of the slipstream effects
  // MAX_DIRTY_AIR: 0.0001,
  // DIRTY_AIR_DEFAULT: 0,
  // DIRTY_AIR_DISTANCE: 200,

  // ===============================
  // Rain and Grip
  // ===============================
  SLIDE_FACTOR: 2.5, // Rain slide multiplier
  
  // ===============================
  // Tire Management
  // ===============================
  MANAGE_TYRES_PENALTY: 0.0001, // Grip reduction when managing tires
  MANAGE_TYRES_WEAR_REDUCTION: 0.8, // 20% wear reduction when managing tires
  TYRES_BLOWED: 0.0007, // Grip reduction when tire is blown (increased for testing)
  // ===============================
  SANDBAG_PENALTY: 0.001,
  TOTAL_DAMAGE_PENALTY: 0.001, // Penalty for 100 damage
  DAMAGE_PENALTY_EXPONENT: 2, // Higher values make low damage less punishing while keeping 100 damage unchanged
};

export function changeConstant(key: keyof typeof constants, value: number) {
  if (key in constants) {
    constants[key] = value;
  }
}
